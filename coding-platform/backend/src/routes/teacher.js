...
// GET /api/teacher/analytics - Frontend expects this endpoint
router.get(
  "/analytics",
  verifyToken,
  requireRole("teacher"),
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      FROM questions q LEFT JOIN users u ON q.created_by = u.id ORDER BY q.id`,
      );
      res.json(result.rows);
    } catch (err) {
      next(err);
